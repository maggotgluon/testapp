<x-layouts.app title="Login">
    <div class="mx-auto max-w-xl rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04] p-6">
        <h1 class="inline-flex items-center gap-2 text-2xl font-semibold text-zinc-950 dark:text-white"><x-icon name="log-in" class="h-6 w-6 text-emerald-500" />Client login / เข้าสู่ระบบลูกค้า</h1>
        ...
        <form method="POST" action="{{ route('login.store') }}" class="mt-6 grid gap-4">
            ...
            <button class="inline-flex items-center justify-center gap-2 rounded-md bg-emerald-400 px-4 py-3 font-semibold text-zinc-950"><x-icon name="check" />Continue / ดำเนินการต่อ</button>
        </form>
    </div>
</x-layouts.app>

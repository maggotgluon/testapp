<x-layouts.app :title="$promotion->exists ? 'Edit promotion' : 'New promotion'">
    <form method="POST" action="{{ $promotion->exists ? route('admin.promotions.update', $promotion) : route('admin.promotions.store') }}" class="mx-auto max-w-3xl rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04] p-6">
        ...
        <div class="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <label class="text-sm text-zinc-700 dark:text-zinc-300">Expires at / หมดอายุ<input class="mt-1 w-full rounded-md border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-950 px-3 py-2 text-zinc-950 dark:text-white" type="datetime-local" name="expires_at" value="{{ old('expires_at', $promotion->expires_at?->format('Y-m-d\TH:i')) }}"></label>

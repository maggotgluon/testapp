<x-layouts.app title="Manage events">
    <div class="flex items-center justify-between gap-3">
        <h1 class="text-3xl font-semibold text-white">Events</h1>
            ...
            <div class="rounded-lg border border-white/10 bg-white/[0.04] p-5">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        ...
                        <div class="text-sm text-zinc-400">{{ $event->starts_at->format('M j, Y H:i') }} · {{ $event->venue }}</div>
                    </div>
                    <div class="flex flex-wrap items-center gap-2">
                        <span class="text-sm text-emerald-200">{{ $event->ticketTypes->count() }} ticket types</span>
                        ...
                        <a class="rounded-md border border-white/10 px-3 py-2 text-sm text-zinc-100" href="{{ route('admin.events.edit', $event) }}">Edit</a>
                    </div>
                </div>
            </div>
        @endforeach

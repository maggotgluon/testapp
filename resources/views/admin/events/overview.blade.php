<x-layouts.app :title="$event->name.' overview'">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <p class="text-sm font-medium uppercase tracking-[0.18em] text-emerald-600 dark:text-emerald-300">Event operations</p>
        ...
        <a class="rounded-md border border-zinc-200 dark:border-white/10 px-3 py-2 text-sm text-zinc-800 dark:text-zinc-100" href="{{ route('admin.events.edit', $event) }}">Edit event</a>
    </div>

    <div class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04] p-5">
            <div class="text-sm text-zinc-600 dark:text-zinc-400">Gross event revenue</div>
            <div class="mt-2 text-3xl font-semibold text-zinc-950 dark:text-white">THB {{ number_format($grossRevenue) }}</div>
        </div>
        <div class="rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04] p-5">
            <div class="text-sm text-zinc-600 dark:text-zinc-400">Orders</div>
            <div class="mt-2 text-3xl font-semibold text-zinc-950 dark:text-white">{{ number_format($orderStatusCounts->sum()) }}</div>
        </div>
        <div class="rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04] p-5">
            <div class="text-sm text-zinc-600 dark:text-zinc-400">Tickets issued</div>
            <div class="mt-2 text-3xl font-semibold text-zinc-950 dark:text-white">{{ number_format($totalTickets) }}</div>
        </div>
        <div class="rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04] p-5">
            <div class="text-sm text-zinc-600 dark:text-zinc-400">Checked in/out</div>
    ...
    <div class="mt-6 grid gap-6 xl:grid-cols-[.8fr_1.2fr]">
        <section class="rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04] p-5">
            <h2 class="text-xl font-semibold text-zinc-950 dark:text-white">Ticket type sales</h2>
                    ...
                    <div class="rounded-md border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-zinc-900 p-4">
                        <div class="flex items-center justify-between gap-3">
                            <div>
                                ...
                                <div class="text-sm text-zinc-600 dark:text-zinc-400">{{ number_format($stat['quantity']) }} sold @if($stat['capacity']) / {{ number_format($stat['capacity']) }} capacity @endif</div>
                            </div>
        ...
        <section class="grid gap-4 sm:grid-cols-2">
            <div class="rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04] p-5">
                <h2 class="text-xl font-semibold text-zinc-950 dark:text-white">Order status</h2>
            ...
            <div class="rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04] p-5">
                <h2 class="text-xl font-semibold text-zinc-950 dark:text-white">Ticket/check-in status</h2>
                <div class="mt-4 grid gap-2 text-sm">
    ...
    <section class="mt-6 rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04]">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-200 dark:border-white/10 p-4">
            <h2 class="text-xl font-semibold text-zinc-950 dark:text-white">Orders for this event</h2>
            ...
            @forelse($orders as $order)
                <div class="grid gap-4 p-4 lg:grid-cols-[1fr_auto]">
                    ...
                    <div class="flex flex-wrap items-start gap-2">
                        ...
                        <form method="POST" action="{{ route('admin.orders.refund', $order) }}">@csrf<button class="rounded-md border border-zinc-200 dark:border-white/10 px-3 py-2 text-sm text-zinc-800 dark:text-zinc-100">Refund</button></form>
                    </div>
                </div>
            @empty
                <div class="p-6 text-zinc-600 dark:text-zinc-400">No orders found for this event.</div>
            @endforelse
        </div>
    ...
    <section class="mt-6 rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04]">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-200 dark:border-white/10 p-4">
            <h2 class="text-xl font-semibold text-zinc-950 dark:text-white">Tickets and check-in</h2>
            <form class="flex gap-2">
        ...
        <div class="divide-y divide-white/10">
            ...
            @empty
                <div class="p-6 text-zinc-600 dark:text-zinc-400">No tickets found for this event.</div>
            @endforelse
        </div>

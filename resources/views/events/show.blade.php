    ...
    <div class="grid gap-8 lg:grid-cols-[.8fr_1.2fr]">
        ...
        <form method="POST" action="{{ route('orders.store') }}" enctype="multipart/form-data" class="rounded-lg border border-white/10 bg-white/[0.04] p-5" x-data="checkout({
                ...
                @foreach($event->ticketTypes as $ticket)
                    <div class="rounded-md border border-white/10 bg-zinc-900 p-4">
                        <div class="flex items-center justify-between gap-4">
                            ...
                            <input class="w-20 rounded-md border border-white/10 bg-zinc-950 px-3 py-2 text-white" name="items[{{ $loop->index }}][quantity]" type="number" min="0" max="20" value="0" x-model.number="quantities[{{ $ticket->id }}]">
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="mt-5 grid gap-4 sm:grid-cols-2">
                <label class="text-sm text-zinc-300">Name<input class="mt-1 w-full rounded-md border border-white/10 bg-zinc-950 px-3 py-2 text-white" name="customer_name" value="{{ auth()->user()->name ?? old('customer_name') }}" required></label>
                <label class="text-sm text-zinc-300">Phone<input class="mt-1 w-full rounded-md border border-white/10 bg-zinc-950 px-3 py-2 text-white" name="customer_phone" value="{{ auth()->user()->phone ?? old('customer_phone') }}" required></label>
                ...
                <label class="text-sm text-zinc-300">Coupon<input class="mt-1 w-full rounded-md border border-white/10 bg-zinc-950 px-3 py-2 uppercase text-white" name="coupon_code" placeholder="EARLYBIRD"></label>
            </div>
            ...
            <div class="mt-4 grid gap-4 sm:grid-cols-2">
                <label class="text-sm text-zinc-300">Payment method<select class="mt-1 w-full rounded-md border border-white/10 bg-zinc-950 px-3 py-2 text-white" name="payment_method" x-model="paymentMethod"><option value="bank_transfer">Direct bank transfer</option><option value="qr_payment">QR payment</option></select></label>
                <label class="text-sm text-zinc-300">Slip image<input class="mt-1 w-full rounded-md border border-white/10 bg-zinc-950 px-3 py-2 text-white" name="slip" type="file" accept="image/*"></label>
            </div>

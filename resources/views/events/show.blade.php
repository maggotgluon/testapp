    ...
    <div class="grid gap-8 lg:grid-cols-[.8fr_1.2fr]">
        ...
        <form method="POST" action="{{ route('orders.store') }}" enctype="multipart/form-data" class="rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04] p-5" x-data="checkout({
            ...
            <div class="mt-4 grid gap-3">
                ...
                @empty
                    <div class="rounded-md border border-amber-400/30 bg-amber-400/10 p-4 text-sm text-amber-800 dark:text-amber-100">No ticket types are on sale right now.</div>
                @endforelse
            </div>

            <div class="mt-5 grid gap-4 sm:grid-cols-2">
                <label class="text-sm font-medium text-zinc-700 dark:text-zinc-200">Name <span class="rounded bg-rose-400/20 px-1.5 py-0.5 text-xs text-rose-700 dark:text-rose-200">required</span><input class="mt-1 w-full rounded-md border border-emerald-400/40 bg-white dark:bg-zinc-950 px-3 py-2 text-zinc-950 dark:text-white focus:border-emerald-300 focus:outline-none" name="customer_name" value="{{ auth()->user()->name ?? old('customer_name') }}" required></label>
                <label class="text-sm font-medium text-zinc-700 dark:text-zinc-200">Phone <span class="rounded bg-rose-400/20 px-1.5 py-0.5 text-xs text-rose-700 dark:text-rose-200">required</span><input class="mt-1 w-full rounded-md border border-emerald-400/40 bg-white dark:bg-zinc-950 px-3 py-2 text-zinc-950 dark:text-white focus:border-emerald-300 focus:outline-none" name="customer_phone" value="{{ auth()->user()->phone ?? old('customer_phone') }}" required></label>
                <label class="text-sm text-zinc-700 dark:text-zinc-300">Email<input class="mt-1 w-full rounded-md border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-950 px-3 py-2 text-zinc-950 dark:text-white" name="customer_email" value="{{ auth()->user()->email ?? old('customer_email') }}"></label>
                @if($event->coupons->isNotEmpty())
                    <label class="text-sm text-zinc-700 dark:text-zinc-300">Coupon<input class="mt-1 w-full rounded-md border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-950 px-3 py-2 uppercase text-zinc-950 dark:text-white" name="coupon_code" placeholder="EARLYBIRD" x-model="couponCode"></label>
                @endif
            </div>
            ...
            @if($event->coupons->isNotEmpty())
                <div class="mt-4 rounded-md border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-zinc-900 p-4 text-sm text-zinc-700 dark:text-zinc-300">
                    <div class="font-medium text-zinc-950 dark:text-white">Available coupons</div>
            ...
            <div class="mt-5 rounded-md border border-emerald-400/20 bg-emerald-400/10 p-4 text-sm text-emerald-950 dark:text-emerald-50">
                ...
                <dl class="mt-3 grid gap-1 text-sm">
                    <div class="flex justify-between gap-3"><dt class="text-emerald-700 dark:text-emerald-200">Subtotal</dt><dd>THB <span x-text="subtotal().toLocaleString()"></span></dd></div>
                    <div class="flex justify-between gap-3"><dt class="text-emerald-700 dark:text-emerald-200">Coupon discount</dt><dd>- THB <span x-text="discount().toLocaleString()"></span></dd></div>
                </dl>
                <template x-if="paymentMethod === 'bank_transfer'">
                    <dl class="mt-3 grid gap-2">
                        <div><dt class="text-emerald-700 dark:text-emerald-200">Bank</dt><dd x-text="payment.bank_name || 'Set bank name in admin event settings'"></dd></div>
                        <div><dt class="text-emerald-700 dark:text-emerald-200">Account name</dt><dd x-text="payment.bank_account_name || '-'"></dd></div>
                        <div><dt class="text-emerald-700 dark:text-emerald-200">Account number</dt><dd class="font-mono" x-text="payment.bank_account_number || '-'"></dd></div>
                    </dl>
                </template>
                <template x-if="paymentMethod === 'qr_payment'">
                    <div class="mt-3 grid gap-4 sm:grid-cols-[160px_1fr]">
                        ...
                        <dl class="grid gap-2">
                            <div><dt class="text-emerald-700 dark:text-emerald-200">QR account</dt><dd x-text="payment.qr_payment_account_name || '-'"></dd></div>
                            <div><dt class="text-emerald-700 dark:text-emerald-200">PromptPay / account</dt><dd class="font-mono" x-text="payment.qr_payment_account || '-'"></dd></div>
                            <div><dt class="text-emerald-700 dark:text-emerald-200">Amount</dt><dd>THB <span x-text="total().toLocaleString()"></span></dd></div>
                            <template x-if="payment.qr_payment_image">
                                <div><dt class="text-emerald-700 dark:text-emerald-200">Reference QR image</dt><dd><a class="underline" :href="payment.qr_payment_image" target="_blank">Open uploaded account QR</a></dd></div>
                            </template>
                        </dl>
                    </div>
                </template>

    ...
    <div class="grid gap-6 lg:grid-cols-[1fr_.75fr]">
        <section class="rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04] p-6">
            ...
            <dl class="mt-5 grid gap-2 text-sm">
                <div class="flex justify-between"><dt class="text-zinc-600 dark:text-zinc-400">Subtotal</dt><dd>THB {{ number_format($order->subtotal_thb) }}</dd></div>
                <div class="flex justify-between"><dt class="text-zinc-600 dark:text-zinc-400">Discount</dt><dd>THB {{ number_format($order->discount_thb) }}</dd></div>
                <div class="flex justify-between text-lg font-semibold text-zinc-950 dark:text-white"><dt>Total</dt><dd>THB {{ number_format($order->total_thb) }}</dd></div>
            </dl>
        </section>
        <section class="rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/[0.04] p-6">

<x-layouts.app title="Find order">
    <div class="mx-auto max-w-4xl">
        <form class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-white/10 dark:bg-white/[0.04]">
            ...
            <button class="mt-5 inline-flex items-center gap-2 rounded-md bg-emerald-400 px-4 py-2 font-semibold text-zinc-950"><x-icon name="search" />Search / ค้นหา</button>
        </form>
            ...
            @foreach($orders as $order)
                <div class="rounded-lg border border-zinc-200 bg-white p-5 dark:border-white/10 dark:bg-white/[0.04]">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        ...
                        @if($isAdmin)
                            <div class="flex flex-wrap gap-2" x-data="{ editStatus: false }">
                                ...
                                @elseif($order->status === 'approved')
                                    <button class="inline-flex items-center gap-2 rounded-md border border-zinc-200 px-3 py-2 text-sm font-semibold text-zinc-800 dark:border-white/10 dark:text-zinc-100" type="button" @click="editStatus = !editStatus"><x-icon name="pencil" /><span x-text="editStatus ? 'Hide / ซ่อน' : 'Edit status / แก้ไขสถานะ'"></span></button>
                                    <div class="flex flex-wrap gap-2" x-cloak x-show="editStatus" x-transition>
                                        ...
                                        <form method="POST" action="{{ route('admin.orders.refund', $order) }}" onsubmit="return confirm('Refund this approved order? / ยืนยันคืนเงินออเดอร์นี้?')">@csrf<button class="inline-flex items-center gap-2 rounded-md border border-zinc-200 px-3 py-2 text-sm font-semibold text-zinc-800 dark:border-white/10 dark:text-zinc-100"><x-icon name="undo" />Refund / คืนเงิน</button></form>
                                    </div>
                    ...
                    <div class="mt-4 grid gap-2">
                        @foreach($order->tickets as $ticket)
                            <div class="flex flex-wrap items-center justify-between gap-3 rounded-md bg-zinc-50 px-3 py-2 text-sm dark:bg-zinc-900">
                                <a class="inline-flex items-center gap-2 text-zinc-800 hover:text-emerald-700 dark:text-zinc-100 dark:hover:text-emerald-200" href="{{ route('tickets.show', ['uuid' => $ticket->uuid, 'phone' => $ticket->holder_phone]) }}"><x-icon name="ticket" />{{ $ticket->event->name }} · {{ $ticket->ticketType->name }} · {{ $ticket->holder_name }}</a>
